...
function demoPlans(): SubscriptionPlan[] {
  return [
    ...
    { plan_key: "agro_productor", display_name: "Productor · EcoNexo AG + EcoCampo", description: "Lotes, fenología, balance hídrico y ventanas de aplicación sobre datos meteorológicos reales.", price_min_usd: 400, price_max_usd: 400, billing_period: "monthly", duration_days: null, entitlements: { max_users: 8, max_devices: 25, max_zones: 10, max_rules: 25, max_critical_layers: 3, municipality_limit: 3, report_frequency: "quincenal", api_access: false, audit_export: true, custom_models: false, sla: false, included_modules: ["core", "agro", "ecocampo"] } },
...
export async function demoGet<T>(path: string): Promise<T> {
  ...
  if (path.startsWith("/ecocampo/")) throw new Error("EcoCampo requiere conexión a la API para evaluar lotes con datos reales.");
