...
export default function MapView({ devices, alerts, detections, reports, center, earth, onAlert, initialSatelliteMode = "NONE", sourceSettings, showForestryAssets = false }: Props) {
  ...
  useEffect(() => {
    ...
    void import("leaflet").then(({ default: L }) => {
      ...
      // Leaflet no detecta por sí solo los cambios de tamaño del contenedor
      // (por ejemplo al retraer/expandir el panel de alertas), así que
      // observamos el contenedor y revalidamos el tamaño para que el mapa
      // siempre complete todo el campo sin quedar en un marco al costado.
      let resizeFrame = 0;
  ...
  useEffect(() => {
    ...
    void import("leaflet").then(({ default: L }) => {
      ...
      // El GeoJSON del backend puede llegar con geometría nula o malformada;
      // en ese caso Leaflet lanza "Invalid GeoJSON object" y crashea la página.
      // Intentamos dibujarlo y, si falla, caemos al polígono local como fallback.
      let territoryDrawn = false;
      if (territoryFeature?.geometry) {
        try {
