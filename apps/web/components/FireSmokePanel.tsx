  ...
  return (
    <div className="view fire-module-view">
      ...
      <section className="fire-map-shell">
        <MapView token={token} devices={devices} alerts={fireAlerts} detections={recent.map((entry) => entry.item)} reports={[]} zones={zones.filter((zone) => zone.kind === "incendio" || zone.kind === "general")} center={center} earth={earth} sourceSettings={sourceSettings} initialSatelliteMode="NONE" availableLayers={["NBR_RAW"]} />
      </section>
