...
async def current_user(authorization: str = Header(default="")) -> CurrentUser:
    ...
    row = await db.pool().fetchrow(
        "SELECT email, role, is_active FROM users WHERE id=$1 AND org_id=$2",
        user_id,
        org_id,
    )
...
async def require_platform_admin(user: CurrentUser = Depends(current_user)) -> CurrentUser:
    if not user.platform_admin or user.role != "admin":
        raise HTTPException(status.HTTP_403_FORBIDDEN, "Se requiere administrador comercial de EcoNexo")
    return user
