...
async def register_organization(body: RegisterIn) -> TokenOut:
...
async def register_community(body: CommunityRegisterIn) -> TokenOut:
    if not body.terms_accepted:
        raise HTTPException(422, "Debés aceptar los términos y la política de privacidad")
    _validate_password(body.password)
    email = str(body.email).strip().lower()
...
async def google_auth(body: GoogleAuthIn) -> TokenOut:
...
async def change_password(body: ChangePasswordIn, user: CurrentUser = Depends(current_user)) -> Response:
